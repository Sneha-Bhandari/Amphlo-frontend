  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    try {
      ...
      if (data && data.id) {
        ...
        try {
          response = await patchdata(`service-offerings/${section.apiPath}`, payload);
        } catch (err) {
          console.log("First endpoint failed, trying with ID:", err);
          response = await patchdata(`service-offerings/${data.id}`, payload);
        }
        ...
        console.log("Creating new service-offerings");
        response = await postdatas("service-offerings", payload);
...
export default function ServicesOfferingsCms() {
  ...
  const offeringsSection = [
    { name: "Partner", displayPath: "partner", apiPath: "partner" },
    { name: "University", displayPath: "university", apiPath: "university" },
  ];
  ...
  const fetchAllOfferings = async () => {
    ...
    setError(null);
    try {
      console.log("Fetching all service-offerings from: service-offerings");
      const res = await getdata("service-offerings");
      console.log("Fetched all service-offerings:", res);
